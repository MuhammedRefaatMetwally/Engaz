...
class ValidateCurrentAddressUseCase @Inject constructor()  {

    operator fun invoke(currentAddress: String, context: Context): Resource<Boolean?> {

        if (currentAddress.isBlank()) {
            return Resource.FailureData(
                Failure(
                    message = "ادخل عنوان الحالي",
                    screenIdInt = 0,
                    exceptionCode = 0,
                    customCode = 0
                )
        ...
        val regex = Regex("[!@#\$%^&*()_+\\-=\\[\\]{};':\"\\\\|,.<>?/~]")
        if (currentAddress.contains(regex)) {
            return Resource.FailureData(
                Failure(
